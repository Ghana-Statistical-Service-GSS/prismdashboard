...
type Photo = {
  id: string; image_url: string | null; captured_at: string | null; created_at: string; synced_at: string | null;
  item_code: string; item_name: string; product_name: string; price: number | null; weight: number | null;
...
export default function PhotosPage() {
  ...
  }, [page, pageSize, regionId, districtId, marketId, itemId, userId, appliedSearch, sort, refreshKey]);
        ...
        <main className="px-5 py-7 md:px-8 xl:px-10">
          ...
          <section className="mt-7 rounded-3xl border border-prism-border/70 bg-white p-5 shadow-sm">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
              ...
              <label className="text-[10px] font-bold uppercase tracking-[0.13em] text-prism-muted">Sort order<select value={sort} onChange={(event) => { setSort(event.target.value); resetPage(); }} className="mt-1 w-full rounded-xl border border-prism-border bg-white px-3 py-2 text-xs font-medium normal-case tracking-normal outline-none focus:border-prism-purple"><option value="balanced">Balanced by region and market</option><option value="latest_synced">Latest pictures synced</option></select></label>
...
function PhotoModal({ photo, onClose }: { photo: Photo; onClose: () => void }) {
  ...
  const uom = photo.uom_standard || photo.uom_local || "No UOM";
  const priced = photo.price != null && Number(photo.price) > 0;
  return <div role="dialog" aria-modal="true" aria-label={`${photo.item_name} photo details`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-5xl overflow-auto rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-prism-border px-5 py-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-prism-teal">Commodity evidence · {priced ? "Priced" : "Not priced"}</p><h2 className="mt-1 text-xl font-black text-prism-text">{photo.item_name} · {photo.product_name}</h2></div><button onClick={onClose} aria-label="Close photo" className="rounded-full bg-prism-bg px-4 py-2 text-lg font-bold text-prism-purple">×</button></div><div className="grid lg:grid-cols-[1.45fr_0.75fr]"><div className="flex min-h-72 items-center justify-center bg-slate-950">{photo.image_url && !failed ? <img src={photo.image_url} alt={`${photo.item_name} — ${photo.product_name}`} decoding="async" referrerPolicy="no-referrer" onError={() => setFailed(true)} className="max-h-[72vh] w-full object-contain" /> : <p className="p-12 text-center text-sm text-white/70">Photo unavailable from the configured storage</p>}</div><div className="space-y-4 p-6"><Detail label="Pricing status" value={priced ? `Priced · ${money.format(Number(photo.price))}` : "Not priced"} strong />{photo.notes && <Detail label="Product description" value={photo.notes} />}<Detail label="Weight and UOM" value={photo.weight == null ? uom : `${number.format(photo.weight)} ${uom}`} /><Detail label="Region" value={photo.region_name} /><Detail label="District / Market" value={`${photo.district_name} · ${photo.market_name}`} /><Detail label="Outlet" value={`${photo.outlet_name}${photo.outlet_code ? ` (${photo.outlet_code})` : ""}`} /><Detail label="Market reader" value={photo.reader_name} /><Detail label="Captured" value={formatDate(photo.captured_at || photo.created_at)} /><Detail label="Synced" value={formatDate(photo.synced_at || photo.created_at)} /><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-prism-muted">Approval status</p><ApprovalBadges photo={photo} /></div></div></div></div></div>;
}

function ApprovalBadges({ photo }: { photo: Photo }) {
  return <div className="mt-3 flex flex-wrap gap-1.5" aria-label="Approval status"><ApprovalBadge label="Supervisor" status={photo.supervisor_approval} /><ApprovalBadge label="RS" status={photo.rs_approval} /><ApprovalBadge label="HQ" status={photo.hq_approval} /></div>;
}

function ApprovalBadge({ label, status }: { label: string; status: string | null }) {
  const normalized = String(status || "PENDING").toUpperCase();
  const approved = normalized === "APPROVED";
  const rejected = normalized === "REJECTED";
  const style = approved ? "border-emerald-200 bg-emerald-50 text-emerald-700" : rejected ? "border-red-200 bg-red-50 text-red-700" : "border-amber-200 bg-amber-50 text-amber-700";
  const icon = approved ? "✓" : rejected ? "×" : "•";
  const state = approved ? "Approved" : rejected ? "Rejected" : "Pending";
  return <span title={`${label}: ${state}`} className={`rounded-full border px-2.5 py-1 text-[10px] font-black ${style}`}>{icon} {label} {state}</span>;
}
