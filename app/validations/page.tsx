...
function ValidationContent({ data }: { data: ValidationData }) {
  ...
  const isSupervisor = data.scope.level === "MARKETS";
  return (
    <>
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Metric label={isSupervisor ? "Assigned regions with gaps" : isRegional ? "Assigned region" : "Regions with missing markets"} value={isRegional ? 1 : data.market_gaps.regions.length} tone="pink" />
        ...
        <Metric label="Readers without submissions" value={data.reader_gaps.readers.length} tone="red" />
      </section>
...
function Metric({ label, value, tone }: { label: string; value: number; tone: "pink" | "purple" | "teal" | "red" }) {
  if (tone === "red") {
    return <article className="relative overflow-hidden rounded-3xl border border-red-700 bg-red-600 p-5 text-white shadow-sm"><span className="absolute right-4 top-4 rounded-full bg-white/15 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em]">Urgent</span><p className="text-[11px] font-bold uppercase tracking-[0.15em] text-white/80">{label}</p><p className="mt-3 text-3xl font-black text-white">{num.format(value)}</p></article>;
  }
...
function MarketGapTable({ regions, markets, scopeRegionId, scopeRegionName, marketScoped }: { regions: GapRow[]; markets: GapRow[]; scopeRegionId: string | null; scopeRegionName: string | null; marketScoped: boolean }) {
  const [regionId, setRegionId] = useState<string | null>(scopeRegionId);
  const [national, setNational] = useState(marketScoped);
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-3xl border border-prism-border/70 bg-white shadow-sm">
      <div className="flex flex-col gap-4 border-b border-prism-border/70 p-5 md:flex-row md:items-center md:justify-between"><div><h2 className="text-base font-black text-prism-text">Markets without initial prices</h2><p className="mt-1 text-xs text-prism-muted">{marketScoped ? "Your assigned markets without initial prices." : selectedRegionName ? `${selectedRegionName}: districts and their markets without prices.` : national ? "All markets without initial prices nationwide." : "Select a region or open the national markets view."}</p></div><div className="flex gap-2">{selectedRegion && !scopeRegionId && !marketScoped && <button onClick={() => { setRegionId(null); setPage(1); }} className="rounded-full bg-prism-bg px-4 py-2 text-xs font-bold text-prism-purple">← All regions</button>}{!marketScoped && !selectedRegionName && <><button onClick={() => { setNational(false); setPage(1); }} className={`rounded-full px-4 py-2 text-xs font-bold ${!national ? "bg-prism-purple text-white" : "bg-prism-bg text-prism-purple"}`}>Regional summary</button><button onClick={() => { setNational(true); setPage(1); }} className={`rounded-full px-4 py-2 text-xs font-bold ${national ? "bg-prism-purple text-white" : "bg-prism-bg text-prism-purple"}`}>National markets</button></>}</div></div>
...
function ReaderGapTable({ regions, readers, scopeRegionId, scopeRegionName, marketScoped }: { regions: GapRow[]; readers: GapRow[]; scopeRegionId: string | null; scopeRegionName: string | null; marketScoped: boolean }) {
  const [regionId, setRegionId] = useState<string | null>(scopeRegionId);
  const [national, setNational] = useState(marketScoped);
  const [filterRegionId, setFilterRegionId] = useState("");
  const [filterMarketId, setFilterMarketId] = useState("");
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-3xl border border-prism-border/70 bg-white shadow-sm">
      <div className="flex flex-col gap-4 border-b border-red-700 bg-red-600 p-5 text-white md:flex-row md:items-center md:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-base font-black text-white">Readers without submissions</h2><span className="rounded-full bg-white/15 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-white">Urgent action required</span></div><p className="mt-1 text-xs text-white/80">{marketScoped ? "Readers without submissions in your assigned markets." : selectedRegionName ? `${selectedRegionName}: readers and their district/market assignments.` : national ? "All readers without submissions nationwide." : "Select a region or open the national reader view."}</p></div><div className="flex gap-2">{selectedRegion && !scopeRegionId && !marketScoped && <button onClick={() => { setRegionId(null); setPage(1); }} className="rounded-full bg-white px-4 py-2 text-xs font-bold text-red-700">← All regions</button>}{!marketScoped && !selectedRegionName && <><button onClick={() => { setNational(false); setPage(1); }} className={`rounded-full px-4 py-2 text-xs font-bold ${!national ? "bg-white text-red-700" : "bg-red-800 text-white"}`}>Regional summary</button><button onClick={() => { setNational(true); setPage(1); }} className={`rounded-full px-4 py-2 text-xs font-bold ${national ? "bg-white text-red-700" : "bg-red-800 text-white"}`}>National readers</button></>}</div></div>
    ...
    <section className="mt-8 overflow-hidden rounded-3xl border border-prism-border/70 bg-white shadow-sm">
      <div className="border-b border-prism-border/70 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div><h2 className="text-base font-black text-prism-text">Item and product price comparison</h2><p className="mt-1 text-xs text-prism-muted">Only price exceptions are shown. Products must share the same item, UOM and market, with equal weights or average weights within 10%; the largest direct product-to-product price gap must be greater than 25%.</p></div>
