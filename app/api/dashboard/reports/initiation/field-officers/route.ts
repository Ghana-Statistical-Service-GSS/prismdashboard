...
export async function GET() {
  ...
  try {
    const response = await dashboardBackendRequest("/reports/initiation/field-officers", { headers: { Authorization: `Bearer ${token}` } });
    return NextResponse.json(await readBackendResponse(response));
  } catch (error) {
    const status = error instanceof BackendError ? error.status : 503;
    return NextResponse.json({ error: { message: error instanceof Error ? error.message : "Field officer report unavailable" } }, { status });
  }
}
