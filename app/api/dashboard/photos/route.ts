...
const allowed = ["page", "pageSize", "regionId", "districtId", "marketId", "itemId", "userId", "quoteId", "search", "sort", "includeFilters"];
...
export async function GET(request: Request) {
  ...
  try {
    const response = await dashboardBackendRequest(`/photos?${outgoing}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    return NextResponse.json({ error: { message: error instanceof Error ? error.message : "Photo album unavailable" } }, { status });
  }
}
