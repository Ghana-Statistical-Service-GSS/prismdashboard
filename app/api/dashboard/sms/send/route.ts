...
export async function POST(request: Request) {
  ...
  try {
    const response = await dashboardBackendRequest("/sms/send", {
    ...
    return NextResponse.json({ error: { message: error instanceof Error ? error.message : "SMS sending failed" } }, { status });
  }
}
