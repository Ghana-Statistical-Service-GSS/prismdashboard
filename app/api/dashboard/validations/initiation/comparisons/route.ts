...
export async function GET(request: Request) {
  ...
  try {
    const response = await dashboardBackendRequest(`/validations/initiation/comparisons?${outgoing}`, { headers: { Authorization: `Bearer ${token}` } });
    return NextResponse.json(await readBackendResponse(response));
  } catch (error) {
    const status = error instanceof BackendError ? error.status : 503;
    return NextResponse.json({ error: { message: error instanceof Error ? error.message : "Comparisons unavailable" } }, { status });
  }
}
