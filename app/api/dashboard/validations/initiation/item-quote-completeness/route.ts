...
const allowed = ["page", "pageSize", "search", "status", "regionId", "marketId", "userId"];
...
export async function GET(request: Request) {
  ...
  try {
    const response = await dashboardBackendRequest(
      `/validations/initiation/item-quote-completeness?${outgoing}`,
    ...
    return NextResponse.json(
      { error: { message: error instanceof Error ? error.message : "Item quote report unavailable" } },
      { status }
    );
  }
}
